...
class Line extends Component {
  ...
  render() {
    return (
      <tr>
        <td>
          <img src={this.props.celeb.pictureUrl} width="50px" />
        </td>
        <td>
          {this.props.celeb.name} - {this.isEven(this.props.celeb.name)}
        </td>
        <td>{this.props.celeb.popularity}</td>
        <td>
          <button onClick={() => this.props.clickDelete(this.props.celeb.name)}>
            Delete
          </button>
          {/* <button onClick={() => this.props.clickDelete(this.props.celeb.name)}>
            Delete
...
class App extends Component {
  ...
  //   const newMovieList = this.state.listContacts;
  //   newMovieList.push(newContact);
  //   this.setState({ listContacts: newMovieList });
  // }

  sortByName() {
    this.state.listContacts.sort((celeb1, celeb2) => {
      if (celeb1.name > celeb2.name) {
        return 1;
      } else if (celeb1.name < celeb2.name) return -1;
      else return 0;
    });
    this.setState({ listContacts: this.state.listContacts });
  }
  ...
  sortByPopularity() {
    this.state.listContacts.sort((celeb1, celeb2) => {
      ...
      else if (celeb1.popularity < celeb2.popularity) return -1;
      else return 0;
    });
    this.setState({ listContacts: this.state.listContacts });
  }
  ...
  deleteLine(name) {
    this.state.listContacts = this.state.listContacts.filter(
    ...
    return (
      <div className="App">
        ...
        <button onClick={this.toggleTitle.bind(this)}>
          {this.state.displayTitle ? "Hide" : "Show"}
        </button>
